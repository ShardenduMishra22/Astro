/* empty css                                 */
import { c as createComponent, r as renderTemplate, m as maybeRenderHead, a as addAttribute, d as createAstro, b as renderComponent } from '../chunks/astro/server_DaRg_WtQ.mjs';
import 'kleur/colors';
import { H as HOMEPAGE_ARTICLE_LIMIT, $ as $$MainLayout } from '../chunks/MainLayout_Cr5w6wEI.mjs';
import { $ as $$SearchForm } from '../chunks/SearchForm_u0jcCzFn.mjs';
import { $ as $$ArticleCard } from '../chunks/ArticleCard_B_E1y37S.mjs';
import 'clsx';
import { f as formatDate } from '../chunks/Tags_B-j-vdiU.mjs';
...
const $$MostRecentArticle = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="relative inline-block w-full sm:w-auto cursor-pointer rounded-2xl"> <a${addAttribute("/articles/" + article.slug, "href")}> <img${addAttribute(article.data.image, "src")} alt="Article Image" class="w-full h-96 rounded-2xl"> <div class="absolute inset-0 flex flex-col items-center justify-center bg-black opacity-80 hover:opacity-75 transition duration-300 ease-in-out text-white text-center rounded-2xl"> <div> <h2 class="text-2xl font-semibold sm:text-3xl"> ${article.data.title} </h2> <p class="text-xl mt-4">${formatDate(article.data.pubDate)}</p> <!-- Tags with rounded border --> <div class="flex mt-4 justify-center"> ${article.data.tags.map((tag) => renderTemplate`<span class="px-2 py-1 border text-white rounded-full text-xs mr-2">${tag}</span>`)} </div> </div> </div> </a> </div>`;
}, "D:/Coding Stuff Shardendu Mishra/Astro/src/components/MostRecentArticle.astro", void 0);
...
Articles, Stories & Learnings of Shardendu Mishra
...
}, "D:/Coding Stuff Shardendu Mishra/Astro/src/pages/index.astro", void 0);

const $$file = "D:/Coding Stuff Shardendu Mishra/Astro/src/pages/index.astro";
const $$url = "";
